...
stdint=$(lshw -class network | awk '/logical name:/{print $3}' | wc -l)
for((s=1;s<=$stdint;s+=1));
do interface=$(lshw -class network | awk '/logical name:/{print $3}' | awk -v z=$s 'NR==z{print $1; exit}')
if [[ $interface = logic* ]];
then continue ; fi
